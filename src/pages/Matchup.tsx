  ...
  return (
    <section className="space-y-8">
      <header className="space-y-3">
        <p className="inline-flex items-center gap-2 rounded-full border border-cyan-400/30 bg-cyan-500/10 px-3 py-1 text-xs font-medium uppercase tracking-[0.25em] text-cyan-200">
      ...
      <Card className="overflow-hidden border-cyan-400/20 bg-card/70">
        <CardContent className="space-y-6 p-4 sm:p-6">
          <div className="grid items-end gap-4 md:grid-cols-[1fr_auto_1fr]">
            ...
            <div className="flex justify-center">
              <Button
                ...
                className="h-14 w-14 rounded-full border border-cyan-300/40 bg-gradient-to-br from-cyan-500/80 to-blue-500/70 text-base font-bold text-white shadow-[0_0_28px_-8px_rgba(34,211,238,0.9)] transition-transform hover:scale-105 disabled:opacity-50"
          ...
          {myChampion && enemyChampion && myChampion === enemyChampion ? (
            <p className="text-sm text-amber-300">Elige dos campeones distintos para iniciar el analisis.</p>
          ) : null}

          {isErrorChampions ? (
            <p className="text-sm text-red-300">No pudimos cargar el listado de campeones.</p>
          ) : null}
        </CardContent>
      </Card>
      ...
      {isAnalyzing ? (
        <Card className="border-cyan-400/20 bg-card/70">
          <CardContent className="space-y-4 p-6">
            <p className="animate-pulse text-center text-sm font-medium uppercase tracking-[0.2em] text-cyan-200">
